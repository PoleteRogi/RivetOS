...
def render(screen, events, manager):
    ...
    if firstFrame:
        ...
        wallpaperScaled = pygame.transform.scale(wallpaper, (400 * wallpaperScale, 800 * wallpaperScale))

    sin += 0.01

    s = pygame.Surface((400, 800), pygame.SRCALPHA)   # per-pixel alpha              # notice the alpha value in the color
    ...
    s.blit(wallpaperScaled, (400 / 2 - 400 * wallpaperScale / 2, 800 / 2 - 800 * wallpaperScale / 2))
    ...
    for app in apps:
        ...
        x = (column * columnsize + 10 * 3) * iconScale + (columnsize / 2) - (iconsize / 2)
        ...
        pygame.draw.rect(rect, (255, 255, 255), (0, 0, *size), border_radius=100)
        ...
        appNameTextRect.center = (x + (iconsize / 2), y + (iconsize / 2) + iconsize - 20)

        s.blit(appNameText, appNameTextRect)

        outlineSurface = pygame.Surface((400, 800), pygame.SRCALPHA)   # per-pixel alpha              # notice the alpha value in the color

        pygame.draw.rect(outlineSurface, (0, 0, 0), pygame.Rect((column * columnsize + 10 * 3) * iconScale + (columnsize / 2) - (iconsize / 2) - 2, (row * rowsize + 10 * 3) + (rowsize / 2) - (iconsize / 2) - 2, iconsize + 4, iconsize + 4), 2, 100)
    ...
    dock = pygame.draw.rect(dockSurface, (255, 255, 255), (0, 800 - 100 + ((iconScale / 20 * 100)), 400, 100), 400)
